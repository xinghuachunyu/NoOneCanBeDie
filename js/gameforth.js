/*
* @Author: name
* @Date:   2018-07-16 16:11:03
* @Last Modified by:   name
* @Last Modified time: 2018-07-16 16:17:05
*/
...
//      block.style.height=img1.style.height;
        ...
        $("forth").appendChild(img4);
        img4.src="img/skybox.png";
        img4.style.left=$("box").offsetWidth-20+"px"
        img4.style.bottom="0px"
        img4.style.width=randomNum(15,5)+"px"
        img4.style.height=randomNum(40,8)+"px"


        var img5=document.createElement("img");
        $("fifth").appendChild(img5);
        img5.src="img/skybox.png";
        img5.style.left=$("box").offsetWidth-20+"px"
        img5.style.bottom="0px"
        img5.style.width=randomNum(15,5)+"px"
        img5.style.height=randomNum(40,8)+"px"
        ...
        img2.timer=setInterval(function(){
         ...
         img3.timer=setInterval(function(){
          ...
          img4.timer=setInterval(function(){
            vir-=20;
            img4.style.left=$("box").offsetWidth-20+vir+"px"
            if(pillarBounce($("people4"),img4)){
                alert("gameover");
                window.open("index.html")
                clearTimer();
            }
            if(img4.offsetLeft<=0)
            {
               vir=0;
                img4.remove();
                clearInterval(img4.timer);
            }
        },500)
           img5.timer=setInterval(function(){
            fir-=20;
            img5.style.left=$("box").offsetWidth-20+fir+"px"
            if(pillarBounce($("people5"),img5)){
                alert("gameover");
                window.open("index.html")
                clearTimer();
            }
            if(img5.offsetLeft<=0)
            {
               fir=0;
                img5.remove();
                clearInterval(img5.timer);
            }
        },500)
    }
    //柱子的移动事件
